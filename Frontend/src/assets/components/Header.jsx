        ...
        <div className="flex items-center gap-3">
          {user ? (
            <div className="relative">
              ...
              {open && <div className="absolute right-0 top-12 z-50 w-72 rounded-2xl border border-slate-100 bg-white p-5 shadow-xl"><p className="truncate text-lg font-bold text-slate-800">{user.name}</p><p className="mt-1 truncate text-sm text-slate-500">{user.email}</p>{user.role === "admin" && <Link onClick={() => setOpen(false)} to="/admin/dashboard" className="mt-5 block rounded-xl bg-[#2C65F4] px-4 py-3 text-center text-sm font-bold text-white transition hover:bg-blue-700">Admin Dashboard</Link>}<button onClick={handleLogout} className="mt-3 w-full rounded-xl bg-red-50 px-4 py-3 text-sm font-bold text-red-600 transition hover:bg-red-100">Logout</button></div>}
            </div>
          ) : (
            <>
